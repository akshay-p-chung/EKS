pipeline {
	...
	parameters {
		...
		choice(name: 'ServiceBranch', choices: ['main'], description: 'Service branch to deploy')
		extendedChoice(description: '', multiSelectDelimiter: ',', name: 'ServiceName', quoteValue: false, saveJSONParameterToFile: false, type: 'PT_CHECKBOX', value: 'All,sample-app,service-one,service-two', visibleItemCount: 3)
	...
	post {
		always {
			dir("${env.C_DIR}") {
				script {
					if (env.docker_img_created == 'true') {
						sh """
							if [ "$ServiceName" = "All" ]; then
								ServiceName="sample-app,service-one,service-two"
							fi
						...
						echo "Docker image not created, skipping cleanup."
					}
				}
			}
			deleteDir()
